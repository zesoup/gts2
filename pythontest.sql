...
CREATE OR REPLACE FUNCTION pytest( _posx numeric, _posy numeric , pixpermapx numeric, pixpermapy numeric,  zoomx float, zoomy float, transparency bool default False )
	RETURNS void
	language plpythonu
	as $$
                                    try:
                                        ...
                                        cursor = plpy.cursor( plan, [float(_posx)+(float(pixpermapx)*zoomx*0.5), float(_posy)+(float(pixpermapy)*zoomy*0.5), float(pixpermapx)*float(zoomx)*0.75] )
                                        ...
                                        #        return (  (pX-oX)/zX , (pY-oY)/zY)
                                        #        #return ( (( Decimal(p[0])-Decimal(offset[0]))/Decimal(_zoom[0]), (Decimal(p[1])-Decimal(offset[1]))/Decimal(_zoom[1])  ) )
